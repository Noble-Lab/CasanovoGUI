...
/**
 * Modal dialog that collects the information needed to upload a Casanovo de novo result to
 * Limelight: the instance URL, the import key, and the target project ID — plus a required
 * description (seeded with the result file name). The URL, key and last project ID are
 * persisted (the key is masked); they prefill on the next upload.
 *
 * <p>A read-only summary of the auto-detected inputs (mzTab, config source, scan files) is
 * shown so the user can confirm what will be uploaded. Returns {@code true} from
...
public class LimelightDialog {
    ...
    public LimelightDialog(Window owner, Settings settings, String defaultDescription, String inputsSummary) {
        this.owner = owner;
    ...
    /**
     * Show modally. On "Upload", validate, persist URL/key/project ID, capture the field
     * values (available via the getters), and return true.
     */
    public boolean showAndCollect() {
        urlField.setText(settings.getLimelightUrl());
        keyField.setText(settings.getLimelightKey());
        projectIdField.setText(settings.getLimelightProjectId());
        ...
        settings.setLimelightUrl(url);
        settings.setLimelightKey(key);
        settings.setLimelightProjectId(projectId);
        settings.save();
        return true;
    }
